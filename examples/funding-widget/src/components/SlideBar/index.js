...
export default class SlideBar extends Component {
  renderNextGoal(data) {
    ...
    return <NextGoal key={nextGoal.id} x={coord.x} y={coord.y} height={lineMarkerHeight} />;
  }
  ...
  renderMetGoals(data) {
    ...
    return data.metGoals.map(goal => {
      ...
      return <MetGoal key={goal.id} x={coord.x} y={coord.y} height={lineMarkerHeight} />;
    });
  }
  ...
  renderFutureGoals(data) {
    ...
    return data.futureGoals.map(goal => {
      const coord = calcXY(goal.goal / data.goal);
      return <FutureGoal key={goal.id} x={coord.x} y={coord.y} height={lineMarkerHeight} />;
    });
  }
